...
# Define the time window for the data
    ...
    stationary_sensor_meta_data = pd.DataFrame(p.stationary_sensor_meta_data)  # Convert the dictionary to a DataFrame
##########################################################################################
##########################################################################################
##########################################################################################
...
##########################################################################################
    ...
    directory = input_directory  # Replace with your directory path
    df_list = []
...
# Overlay the coordinate pairs on the map as  colored dots
                for idc, row in df_step4.iterrows():
                    ...
                    tooltip_text = f"File: {filename}<br>Time: {row['rtcTime']}<br>Elevation: {row['Altitude']} m <br>correctedTemp: {row['corrected_temperature']} °C <br>rawTemp: {row['degC']} °C  "
                #tooltip=tooltip_text
                    ...
                    # Ensure temperature is valid
                        if pd.isna(row['degC']) or np.isinf(row['degC']):
                            ...
                            continue
                        #print(f"Adding marker: lat={latitude}, lon={longitude}, color={fill_color}, tooltip={tooltip_text}")
                        folium.CircleMarker(
...
# df_step6 defined --- Concatenate all dataframes for the entire run
 ...
 # Add a larger circle to indicate the location of the highest and lowest corrected temperature
    ...
    tooltip_text = (f"File: {filename}<br>Time: {row['rtcTime']}<br>Elevation: {row['Altitude']} m <br>Min corrected Temp: {row['corrected_temperature']} °C <br>rawTemp: {row['degC']} °C ")                                                                                                                       
    folium.CircleMarker(
    ...
    tooltip_text = (f"File: {filename}<br>Time: {row['rtcTime']}<br>Elevation: {row['Altitude']} m <br>Max corrected Temp: {row['corrected_temperature']} °C <br>rawTemp: {row['degC']} °C  ")   
    folium.CircleMarker(
...
# Add stationary sensors to the map
    ...
    for _, sensor in stationary_sensor_meta_data.iterrows():    # _ underscore indicates that the first value of the tuple (the index) is not used
        tooltip_text=(f"Stationary Sensor {sensor['sensor_number']} <br>Temperature: {sensor['temperature']}")
        fill_color = str(colormap(sensor['temperature']))
    ...
    m.save(filename)
    print(f"\n\n Map has been created and saved as {color_coded_temperature_map}\n\n")
